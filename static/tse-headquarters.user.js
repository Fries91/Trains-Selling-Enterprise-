...
// @version      8.0.0
// @description  T.S.E Headquarters hub overlay. Companies, trains, notes, company keys, settings, HoF search.
...
(function () {
  ...
  const DEFAULT_BASE_URL = "https://sevends-hiring-scan.onrender.com";
  ...
  GM_addStyle(`
    ...
    #tse_hq_badge svg{ width:30px; height:30px; filter:drop-shadow(0 4px 10px rgba(0,0,0,.55)); }
    #tse_hq_badge .dot{
  ...
  let ui = gmJsonGet(K_UI, uiDefault);
  if (!ui || typeof ui !== "object") ui = structuredClone ? structuredClone(uiDefault) : JSON.parse(JSON.stringify(uiDefault));
  ...
  function applyUIPositions() {
    ...
    const pw = Math.max(320, pr.width || Math.min(620, window.innerWidth - 98));
    const ph = Math.max(240, pr.height || Math.min(window.innerHeight * 0.8, 700));
  ...
  const badgeMoved = makeDraggable(badge, (x, y) => {
  ...
  function togglePanel() {
    ...
    if (ui.open) closePanel();
      ...
      openPanel();
      fetchState().then(renderBody).catch(() => renderBody());
    }
  }
      ...
      <div class="tse_card">
        <div class="tse_row">
          <div class="tse_field" style="flex:1 1 100%;">
            ...
            <div class="tse_small">Each company shows employee dropdowns and total employee stat summaries. Set company IDs in Settings if this is empty.</div>
          </div>
        </div>
      </div>
      ...
      <div class="tse_card">
        ...
        <div class="tse_row" style="margin-top:10px;">
          ...
          <div class="tse_field" style="flex:2 1 320px;">
            ...
            <div class="tse_small">This uses the server’s cached worker dataset. Refresh it in backend later when you want newer results.</div>
          </div>
        </div>
